...
export class GitClient {
  ...
  private configure = async () => {
    ...
    await this.exec`git config --replace-all ${userEmailKey} "github-actions[bot]@users.noreply.github.com"`;
    ...
    this.registerCleanup(async () => {
      ...
      if (originalAutoSetupRemote) {
        await this.exec`git config --replace-all ${autoSetupRemoteKey} ${originalAutoSetupRemote}`;
      }
    });
