...
export class NpmClient {
   ...
   */
  constructor(private readonly npmToken: string | null, private readonly cwd: string = process.cwd()) {
  ...
  install = async (packageSpecifiers: string[], options: { dev: boolean } = { dev: false }) => {
    await this.execWithIO`npm install ${options.dev ? '-D' : ''} ${packageSpecifiers.join(' ')}`;
  };
